...
class AdminDeviceList extends Page {
    ...
    private function getLayout() {
        ...
        $deskOptions = $this->getComboboxOptions();
        $tdOptions   = $this->getComboboxTdOptions();
...
<div class="layout">
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <a href="$gvPath/application/adminDeviceList" class="nav-item active"><i class="fas fa-mobile-alt"></i><span>Appareils</span></a>
    ...
    <main class="main-content">
        <div class="content-wrapper">
            <div class="page-header">
                ...
                <p class="subtitle">Gérez les appareils d'affichage de votre système</p>
            </div>

            <div class="toast" id="toast"><i class="toast-icon" id="toastIcon"></i><span id="toastMsg"></span></div>

            <div class="card">
                <div class="card-header"><i class="fas fa-mobile-alt"></i><h3>Liste des appareils</h3></div>
                <div class="table-container">
                    <table class="stats-table">
                        <thead><tr><th>Adresse IP</th><th>Fonction</th><th>Domaines thématiques</th><th>Actions</th></tr></thead>
                        <tbody id="devTableBody">$table</tbody>
                    </table>
                </div>
            </div>

            <div class="actions-section">
                <button class="btn btn-primary" onclick="openModal()"><i class="fas fa-plus"></i> Ajouter un appareil</button>
            </div>
        </div>
    </main>
</div>
    ...
    <div class="modal" id="deviceModal">
        <div class="modal-header">
            <div class="modal-title">
                <div class="modal-icon"><i class="fas fa-mobile-alt" id="modalIcon"></i></div>
                <div><h2 id="modalTitle">Ajouter un appareil</h2><p id="modalSubtitle">Remplissez les informations de l'appareil</p></div>
            </div>
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <div class="select-wrapper">
                        <select name="dev_desk_number" id="modal_dev_desk_number" onchange="handleFunctionChange()">
                            $deskOptions
                        </select>
                ...
                <div class="form-group" id="tdGroup">
                    <label for="modal_dev_td_code"><i class="fas fa-folder-tree"></i> Domaine thématique</label>
                    <div class="select-wrapper">
                        <select name="dev_td_code" id="modal_dev_td_code">$tdOptions</select>
                        <i class="fas fa-chevron-down select-arrow"></i>
                    </div>
                </div>
            </div>
    ...
    <div class="modal modal-sm" id="deleteModal">
        <div class="modal-header modal-header-danger">
            <div class="modal-title">
                ...
                <div><h2>Supprimer l'appareil</h2><p>Cette action est irréversible</p></div>
            </div>
            <button class="modal-close" onclick="closeDeleteModal()"><i class="fas fa-times"></i></button>
        </div>
        <div class="modal-body">
            <p class="delete-confirm-text">Confirmez-vous la suppression de l'appareil <strong id="deleteTargetName"></strong> ?</p>
        </div>
        <div class="modal-footer">
...
function openModal(devId, devIp, devDeskNumber, devTdCode) {
    const isEdit = !!devId;
    document.getElementById('modal_dev_id').value          = devId         || 0;
    document.getElementById('modal_dev_ip_address').value  = devIp         || '';
    document.getElementById('modal_dev_desk_number').value = devDeskNumber || '0';
    document.getElementById('modal_dev_td_code').value     = devTdCode     || '0';
    ...
    setTimeout(() => document.getElementById('modal_dev_ip_address').focus(), 300);
}
...
function handleFunctionChange() {
    const v = document.getElementById('modal_dev_desk_number').value;
    document.getElementById('tdGroup').style.display = v !== '0' ? 'none' : 'flex';
    if (v !== '0') document.getElementById('modal_dev_td_code').value = '0';
}
...
function confirmDelete(devId, label, rowEl) {
    pendingDeleteId = devId; pendingDeleteRow = rowEl;
...
document.getElementById('confirmDeleteBtn').addEventListener('click', () => {
    ...
    fetch(gvPath + '/ajax/removeRecord?dev_id=' + pendingDeleteId, { method: 'GET', headers: { 'X-Requested-With': 'XMLHttpRequest' } })
        ...
        .then(() => {
            if (pendingDeleteRow) {
                ...
                setTimeout(() => {
                    ...
                    if (tbody.children.length === 0) tbody.innerHTML = '<tr><td colspan="4" class="empty-row">Aucun appareil disponible</td></tr>';
                }, 350);
            }
            closeDeleteModal(); showToast('Appareil supprimé avec succès', 'success');
        })
...
HTML;
    ...
    private function getTableBody() {
        ...
        if (!count($devices)) return '<tr><td colspan="4" class="empty-row">Aucun appareil disponible</td></tr>';

        $html = "";
        foreach ($devices as $dev) {
            $id  = $dev->getId();
            $ip  = htmlspecialchars($dev->getIpAddress());
            if ($dev->getDeskNumber()) {
                $role    = "Affichage compteur " . $dev->getDeskNumber();
                $tdTxt   = "-";
                $jsCall  = "openModal($id, '$ip', '{$dev->getDeskNumber()}', '0')";
            } else {
                $role    = "Affichage salle";
                $td      = $dev->getTdCode() ? htmlspecialchars($dev->getTdCode()) : '';
                $tdTxt   = $td ?: "Tous";
                $jsCall  = "openModal($id, '$ip', '0', '$td')";
            }
            $html .= <<<HTML
<tr id="dev-row-$id">
    <td>$ip</td>
    ...
    <td class="actions-cell">
        <button class="action-btn edit" onclick="$jsCall" title="Modifier"><i class="fas fa-edit"></i></button>
        <button class="action-btn delete" onclick="confirmDelete($id, '$ip', document.getElementById('dev-row-$id'))" title="Supprimer"><i class="fas fa-trash"></i></button>
    </td>
</tr>
HTML;
        }
    ...
    public function getPageTitle() { return "Gestion des appareils"; }

    private function getComboboxOptions() {
        $ret = '<option value="0">Affichage salle</option>';
        foreach (Desk::getUsedDeskNumbers() as $num) $ret .= "\n<option value=\"$num\">Affichage compteur $num</option>";
        return $ret;
    }

    private function getComboboxTdOptions() {
        $ret = '<option value="0">Tous</option>';
        foreach (TopicalDomain::fromDatabaseCompleteList() as $td) { $code = htmlspecialchars($td->getCode()); $ret .= "\n<option value=\"$code\">$code</option>"; }
        return $ret;
    }
...
<style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .modal { background: #fff; border-radius: 20px; width: 100%; max-width: 520px; box-shadow: 0 24px 60px rgba(108,99,255,0.2), 0 8px 24px rgba(0,0,0,0.12); transform: translateY(20px) scale(0.97); transition: transform 0.3s cubic-bezier(0.34,1.56,0.64,1); overflow: hidden; max-height: 90vh; overflow-y: auto; }
    ...
    .form-group input[type="text"] { width: 100%; padding: 11px 14px; border: 2px solid #eee; border-radius: 10px; font-size: 14px; color: #1a1a2e; font-family: inherit; transition: border-color 0.2s ease, box-shadow 0.2s ease; outline: none; }
    ...
    @media (max-width: 768px) {
        ...
        .modal { max-width: 100%; } .modal-body { padding: 20px; } .modal-footer { padding: 14px 20px 20px; }

    ...
    <div class="modal" id="deviceModal">
        <div class="modal-header">
            <div class="modal-title">
                ...
                <div>
                    <h2 id="modalTitle">Ajouter un appareil</h2>
                    <p id="modalSubtitle">Remplissez les informations de l'appareil</p>
                </div>
            </div>
        ...
        <form id="deviceForm" method="post" action="$gvPath/application/adminDeviceEdit">
            ...
            <div class="modal-body">
                <div class="form-group">
                    <label for="modal_dev_ip_address">
                        <i class="fas fa-wifi"></i> Adresse IP
                    </label>
                    <input type="text" name="dev_ip_address" id="modal_dev_ip_address"
                           placeholder="ex: 192.168.1.100" autocomplete="off" />
                </div>

                <div class="form-group">
                    <label for="modal_dev_desk_number">
                        <i class="fas fa-desktop"></i> Fonction
                    </label>
                    <select name="dev_desk_number" id="modal_dev_desk_number">
                        {$this->getComboboxOptions()}
                    </select>
                </div>

                <div class="form-group">
                    <label for="modal_dev_td_code">
                        <i class="fas fa-folder-tree"></i> Domaine thématique
                    </label>
                    <select name="dev_td_code" id="modal_dev_td_code">
                        {$this->getComboboxTdOptions()}
                    </select>
                </div>
            </div>
...
function openModal(devId, devIp, devDeskNumber, devTdCode) {
    ...
    document.getElementById('modalIcon').className       = isEdit ? 'fas fa-mobile-alt' : 'fas fa-mobile-alt';
    ...
    private function getTableBody() {
        ...
        foreach ($devices as $dev) {
            ...
            if ($dev->getDeskNumber()) {
                ...
                $tdTxt = "-";
                $deskNum = $dev->getDeskNumber();
                $tdCode = '';
            } else {
                $role  = "Affichage salle";
                $tdTxt = $dev->getTdCode() ? htmlspecialchars($dev->getTdCode()) : "Tous";
                $deskNum = '0';
                $tdCode = $dev->getTdCode() ? htmlspecialchars($dev->getTdCode()) : '';
            }
...
<tr>
    ...
    <td class="actions-cell">
        <button class="action-btn edit"
                onclick="openModal($id, '$ip', '$deskNum', '$tdCode')"
                title="Modifier">
...
HTML;
    ...
    private function getComboboxOptions() {
        $ret = '<option value="0">Display de salle</option>';
        foreach ( Desk::getUsedDeskNumbers() as $num ) {
            $ret .= "\n<option value=\"$num\">Display compteur $num</option>";
        }
        return $ret;
    }
